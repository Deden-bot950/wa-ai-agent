...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    var GROQ_API_KEY  = process.env.GROQ_API_KEY;
    var OWNER_NUMBER  = process.env.OWNER_NUMBER || "";
    // FIX: Coba kedua nama variable Redis
    var UPSTASH_URL   = process.env.UPSTASH_REDIS_REST_URL || process.env.UPSTASH_REDIS_URL;
    var UPSTASH_TOKEN = process.env.UPSTASH_REDIS_REST_TOKEN || process.env.UPSTASH_REDIS_TOKEN;
    ...
    history.push({ role: "user", content: actualMessage });
    // Batasi history hanya 10 pesan terakhir
    if (history.length > 10) history = history.slice(history.length - 10);
    ...
    // FIX: Tambah timeout 8 detik untuk Groq
    var groqRes;
    try {
      ...
      // Timeout atau error Groq - kirim pesan fallback
      await fetch("https://api.fonnte.com/send", {
        method: "POST",
        headers: { "Authorization": FONNTE_TOKEN, "Content-Type": "application/json" },
        body: JSON.stringify({ target: sender, message: "Maaf Kaka, Rifty lagi sibuk sebentar. Mohon kirim pesan lagi ya 🙏", countryCode: "62" })
      });
      return { statusCode: 200, headers: headers, body: JSON.stringify({ status: "timeout" }) };
    }

    var groqData = await groqRes.json();
    var aiReply = (groqData.choices && groqData.choices[0] && groqData.choices[0].message && groqData.choices[0].message.content)
      ? groqData.choices[0].message.content
      : "Maaf Kaka, ada gangguan teknis. Silakan coba lagi ya 🙏";

    history.push({ role: "assistant", content: aiReply });

    // Simpan history ke Upstash Redis dengan expiry 7 hari (604800 detik)
    if (UPSTASH_URL && UPSTASH_TOKEN) {
      try {
        await fetch(UPSTASH_URL + "/set/chat:" + cleanSender + "/ex/604800", {
          method: "POST",
          headers: { "Authorization": "Bearer " + UPSTASH_TOKEN, "Content-Type": "application/json" },
          body: JSON.stringify({ value: JSON.stringify(history) })
        });
